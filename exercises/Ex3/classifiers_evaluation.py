...
def load_dataset(filename: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return data[:,:2], data[:,2:].reshape(data.shape[0])
...
def run_perceptron():
    ...
    for n, f in [("Linearly Separable", "linearly_separable.npy"), ("Linearly Inseparable", "linearly_inseparable.npy")]:
        ...
        Perceptron(include_intercept=False, callback=callback).fit(X_train, y_train)
...
def plot_mean_and_cov(ax, estimator):
    # get the estimated mean and cov matrices
    means, cov_list = estimator.classes_mean_cov()

    # plot means
    ax.scatter(means[:, 0], means[:, 1], c='black', marker='x')

    nstd = 2        # ellipse normalization factor
    for mu, co in zip(means, cov_list):
...
def plot_predictions(ax, X, y_true, y_pred, mark_dict, cmap):
    for px, py, pred, true in zip(X[:, 0], X[:, 1], y_pred, y_true):
...
def get_classes_color_and_markers(y):
    ...
    for l in classes:
        plt.scatter([], [], color=cmap.to_rgba(l), marker=markers_dict[l], label=l)
    plt.legend(title="Classes", loc='best', bbox_to_anchor=(0.07, 0.2))
...
def compare_gaussian_classifiers():
    ...
    for f in ["gaussian1.npy", "gaussian2.npy"]:
        ...
        # also legend a classes dictionary
        classes, mark_dict, cmap = get_classes_color_and_markers(y_train)

        fig.suptitle('Dataset:  %s' % f, fontsize=16)

        i = 0
        for name, estimator in [("LDA estimator", LDA_dis), ("GNB Estimator", GAU)]:

            # Fit models and predict over training set
            y_pred, acc = fit_model_and_predict(estimator, X_train, y_train)

            # subplot title
            ax[i].set_title("%s. Accuracy: %.3f" % (name, acc), fontsize=10)

            # Plot the prediction of the estimator
            plot_predictions(ax[i], X_train, y_train, y_pred, mark_dict, cmap)
